...
class EwokParsers extends RegexParsers {
  ...
  //  EXPRESSION ::= DECLARATION | CONDITIONAL | DISJUNCTION
  def expression: Parser[Expression] = declaration | conditional | disjunction | failure("Invalid" +
      " expression")
  ...
  //  CONDITIONAL ::= if~(~EXPRESSION~)~EXPRESSION~(else~EXPRESSION)?
  def conditional: Parser[Conditional] = "if" ~ "(" ~ expression ~ ")" ~ expression ~ opt("else"
      ~> expression) ^^ {
    case "if" ~ "(" ~ condition ~ ")" ~ consequent ~ None => Conditional(condition, consequent)
    case "if" ~ "(" ~ condition ~ ")" ~ consequent ~ Some(alternate) => Conditional(condition,
